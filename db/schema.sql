...
DROP TABLE IF EXISTS service_job;         -- done
DROP TABLE IF EXISTS service_service;     -- done
DROP TABLE IF EXISTS service_employee;    -- done
DROP TABLE IF EXISTS service_vehicle;     -- done
DROP TABLE IF EXISTS service_job_info;    -- done
DROP TABLE IF EXISTS service_customer;    -- done
DROP TABLE IF EXISTS service_address;     -- done
...
CREATE TABLE service_vehicle (
   ...
   -- key setup
   PRIMARY KEY (vin),
   FOREIGN KEY (owner)        REFERENCES service_customer (customer_id)
);
...
INSERT INTO service_customer
   (name_first, name_second, phone_primary, phone_secondary, address_id)
VALUES
   ('Zach', 'Delano', 4252299246, NULL, (SELECT address_id FROM service_address WHERE street='11115 156th PL NE')),
   ('Joseph', 'Shea', 7858503194, 7579333371, (SELECT address_id FROM service_address WHERE street='2526  Sherman Street')),
   ('David', 'Sweet', 9376765240, 7409723894, (SELECT address_id FROM service_address WHERE street='249 Harter Street'));

-- inserting into service_vehicle
INSERT INTO service_vehicle
   (vin, color, make, model, year, owner)
VALUES
   (29471928566213456, 'silver', 'Ford', 'Focus', 2010, (SELECT customer_id FROM service_customer WHERE (name_first='Zach' AND name_second='Delano'))),
   (46598857982385012, 'grey', 'Ford', 'F150', 2017, (SELECT customer_id FROM service_customer WHERE (name_first='Zach' AND name_second='Delano'))),
   (75607300571823011, 'red', 'Jaguar', 'F-TYPE', 2020, (SELECT customer_id FROM service_customer WHERE (name_first='Joseph' AND name_second='Shea'))),
   (89827490182379050, 'red', 'Toyota', 'Prius', 2005, (SELECT customer_id FROM service_customer WHERE (name_first='David' AND name_second='Sweet')));
...
INSERT INTO service_service
   (customer_id, vin, advisor, notes)
VALUES
   (
      (SELECT customer_id FROM service_customer WHERE (name_first='Zach' AND name_second='Delano')),
      (SELECT vin FROM service_vehicle WHERE (owner=(SELECT service_customer.customer_id FROM service_customer WHERE (name_first='Zach' AND name_second='Delano'))) LIMIT 1),
...
SELECT
   (ss.service_id, ss.customer_id, sj.job_name, sj.technician, ji.cost)
FROM
   service_service AS ss
LEFT JOIN service_job      AS sj ON ss.service_id=sj.service_id
LEFT JOIN service_job_info AS ji ON sj.job_name=ji.job_name;
